...
void* my_malloc(size_t size) {
  ...
  //Find a free slot
  while(metadata){
    if(metadata->size == size){   //Try to find an exactly fit free slot, if found, then break
      found = 1;
      break;
    }
    else if(metadata->size > size){ //in case that there is no exactly-fit free slot, find a free slot that has the closest size_t with the required size
      if(det==1){ //first time when we find a possible slot, calculate the initial difference
  ...
  if(found == 1){  //exactly-fit slot found, remove it from the free list
    void* ptr = metadata + 1;
    simple_remove_from_free_list(metadata, prev);
    return ptr;
  }
  else{  // no exactly-fit slot, use the slot that has the closest size to proceed
    prev = temp_prev;
    metadata = temp;
  }
